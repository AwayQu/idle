...
rac_propertyAttributes *rac_copyPropertyAttributes (objc_property_t property) {
    ...
    // allocate enough space for the com.away1994.structure and the type string (plus a NUL)
    rac_propertyAttributes *attributes = calloc(1, sizeof(rac_propertyAttributes) + typeLength + 1);
    if (!attributes) {
        fprintf(stderr, "ERROR: Could not allocate rac_propertyAttributes com.away1994.structure for attribute string \"%s\" for property %s\n", attrString, property_getName(property));
        return NULL;
    }
